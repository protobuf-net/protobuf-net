...
namespace ProtoBuf.BuildTools.Generators
{
    partial class ProtoModelGenerator
    {
        ...
        internal readonly struct SchemaText : IEquatable<SchemaText>
        {
            ...
            /// <summary>
            /// How the DTO generator was told to handle unknown sub-types for this schema
            /// (<c>ProtoBuf_SubTypes</c>), mirrored here so the two generated halves agree.
            /// </summary>
            /// <remarks>
            /// This is deliberately read from the DTO generator's own metadata key rather than
            /// from anything the model declares. The failure mode is asymmetric: over-emitting
            /// the check merely wastes it, while eliding it against DTOs that are neither
            /// <c>sealed</c> nor <c>IgnoreUnknownSubTypes</c> silently stops throwing where
            /// <c>RuntimeTypeModel</c> still throws.
            /// </remarks>
            public SchemaSubTypes SubTypes { get; }

            /// <summary>
            /// Parses the shared option. Mirrors <c>UnknownSubTypeHandling</c> in
            /// protobuf-net.Reflection - which BuildTools compiles in, but this runs over
            /// metadata strings rather than over that enum, so the spellings are matched here.
            /// An unrecognised value is <see cref="SchemaSubTypes.Default"/>, exactly as the DTO
            /// generator treats it: the two must agree on typos as well as on valid values.
            /// </summary>
            public static SchemaSubTypes ParseSubTypes(string? value)
            {
                value = value?.Trim();
                if (string.IsNullOrEmpty(value)) return SchemaSubTypes.Default;
                if (string.Equals("sealed", value, StringComparison.OrdinalIgnoreCase))
                    return SchemaSubTypes.Sealed;
                if (string.Equals("ignore", value, StringComparison.OrdinalIgnoreCase))
                    return SchemaSubTypes.Ignore;
                return SchemaSubTypes.Default;
            }
            ...
            public bool Equals(SchemaText other)
                => Path == other.Path && Content == other.Content
                    && IncludeInOutput == other.IncludeInOutput && SubTypes == other.SubTypes;
            ...
            public override int GetHashCode()
                => ((((Path?.GetHashCode() ?? 0) * 397) ^ (Content?.GetHashCode() ?? 0)) * 397
                    ^ IncludeInOutput.GetHashCode()) * 397 ^ (int)SubTypes;
        }
            ...
            ImmutableArray<SchemaText> schemas, CancellationToken cancellationToken)
        {
            ...
            foreach (var (text, request) in chosen)
            {
                ...
                var built = SchemaPlanBuilder.TryBuild(set, CSharpNames(),
                    plan.Namespace, plan.TypeName, out var unsupported, text.SubTypes);
                if (built is null)
                {
                    diagnostics.Add(new PlanDiagnostic(ProtoDiagnosticKind.SchemaUnsupported,
