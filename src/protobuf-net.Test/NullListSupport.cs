...
message SomePoco {
    int32 Value = 1;
        ...
        [InlineData(false, Scenario.SingleNull, Outcome.AllContentsNullSubst, "22-00")]
        [InlineData(false, Scenario.MultipleWithNull, Outcome.AllContentsNullSubst, "22-02-08-2A-22-00-22-02-08-0C-22-00")]
...
message SomePoco {
    int32 Value = 1;
        ...
        [InlineData(false, Scenario.SingleNotNull, Outcome.AllContents, "12-01-2A")]
...
message SomePoco {
    int32 Value = 1;
        ...
        [InlineData(false, Scenario.SingleNull, Outcome.AllContentsNullSubst, "23-24")]
        [InlineData(false, Scenario.MultipleWithNull, Outcome.AllContentsNullSubst, "23-08-2A-24-23-24-23-08-0C-24-23-24")]
        [InlineData(false, Scenario.SingleNull, Outcome.SerializeFail, "", true)]
        [InlineData(false, Scenario.MultipleWithNull, Outcome.SerializeFail, "", true)]
        [InlineData(true, Scenario.Null, Outcome.Null, "")]
        [InlineData(true, Scenario.Empty, Outcome.Null, "")]
...
message SomePoco {
    int32 Value = 1;
        ...
        private void RunInt32List<T>(bool supportNull, Scenario scenario, Outcome outcome, string expectedHex, bool fullCompile = false) where T : class, IHazLists, new()
        {
            ...
            catch when (outcome is Outcome.SerializeFail)
            {
                return;
            }
        ...
        static void CompareLists<T>(List<T> original, List<T> actual, Outcome outcome, T defaultValue)
        {
            switch (outcome)
            {
                ...
                case Outcome.AllContents:
                    ...
                    break;
                case Outcome.AllContentsNullSubst:
                    Assert.NotNull(actual);
                    Assert.NotSame(actual, original);
                    Assert.True(actual.SequenceEqual(original.Select(x => x ?? defaultValue)));
                    break;
                default:
