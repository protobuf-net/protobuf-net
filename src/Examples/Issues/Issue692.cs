...
namespace ProtoBuf.Issues
{
    public class Issue692
    {
        ...
        public void IgnoringIgnoresInvalidSubType()
        {
            ...
            CheckSuccess(model, @base, "D0-02-2A");
            CheckSuccess(model, derived, "D0-02-2A");
            model.CompileInPlace();
            CheckSuccess(model, @base, "D0-02-2A");
            CheckSuccess(model, derived, "D0-02-2A");
            var compiled = model.Compile();
            CheckSuccess(compiled, @base, "D0-02-2A");
            CheckSuccess(compiled, derived, "D0-02-2A");
        }
        ...
        public void DefaultDetectsInvalidSubType()
        {
            ...
            CheckSuccess(model, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(model, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+DefaultConfigurationDerivedClass");
            model.CompileInPlace();
            CheckSuccess(model, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(model, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+DefaultConfigurationDerivedClass");
            var compiled = model.Compile();
            CheckSuccess(compiled, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(compiled, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+DefaultConfigurationDerivedClass");
        }
        ...
        public void RespectingDetectsInvalidSubType()
        {
            ...
            CheckSuccess(model, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(model, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+RespectingSubTypesDerivedClass");
            model.CompileInPlace();
            CheckSuccess(model, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(model, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+RespectingSubTypesDerivedClass");
            var compiled = model.Compile();
            CheckSuccess(compiled, @base, "D0-02-2A");
            CheckUnexpectedSubTypeDetected(compiled, derived, "Unexpected sub-type: ProtoBuf.Issues.Issue692+RespectingSubTypesDerivedClass");
        }

        static void CheckSuccess<T>(TypeModel model, T obj, string expectedHex)
        ...
        static void CheckUnexpectedSubTypeDetected<T>(TypeModel model, T obj, string expectedError)
            where T : class, IHazFoo
        {
            using var ms = new MemoryStream();
            var ex = Assert.Throws<InvalidOperationException>(() => Serializer.Serialize<T>(ms, obj));
