...
// input sha256 1a84c44eb187774d5487517713c7d0d2d56b5a1646f511f79d20f0251b4bcc62
...
internal sealed class ___PBN_Services___PackedAllModel : ISerializer<EveryPackedShape>
{
	EveryPackedShape ISerializer<EveryPackedShape>.Read(ref ProtoReader.State state, EveryPackedShape value)
	{
		...
		while ((num = state.ReadFieldHeader()) > 0)
		{
			...
			case 17:
			{
				Readings derived = value.Derived;
				derived = RepeatedSerializer.CreateList<Readings, uint>().ReadRepeated(ref state, SerializerFeatures.WireTypeVarint, derived);
				if (derived != null)
				{
	...
	void ISerializer<EveryPackedShape>.Write(ref ProtoWriter.State state, EveryPackedShape value)
	{
		...
		if (derived != null)
		{
			Readings values12 = derived;
			RepeatedSerializer.CreateList<Readings, uint>().WriteRepeated(ref state, 17, SerializerFeatures.WireTypeVarint, values12);
		}
	}
