...
// input sha256 c98e709a0539f7921d305c2827df1f5be2d870f47644359b4bf606d480f54fd7
...
internal sealed class ___PBN_Services___ExternalSerializerModel : ISerializer<Holder>, ISerializer<Panel>, ISerializerProxy<Thing>, ISerializerProxy<Ticket>, ISerializerProxy<Stamp>, ISerializerProxy<Gauge>
{
	...
	SerializerFeatures ISerializer<Panel>.get_Features()
	{
		//ILSpy generated this explicit interface implementation from .override directive in Features_82
		return this.Features_82();
	}

	Panel ISerializer<Panel>.Read(ref ProtoReader.State state, Panel value)
	{
		if (value == null)
		{
		...
		while ((num = state.ReadFieldHeader()) > 0)
		{
			...
			case 1:
			{
				...
				gauge = state.ReadMessage(SerializerFeatures.CategoryRepeated, gauge, SerializerCache.Get<GaugeSerializer, Gauge>());
				if (gauge != null)
				{
					value.Gauge = gauge;
				}
	...
	void ISerializer<Panel>.Write(ref ProtoWriter.State state, Panel value)
	{
		TypeModel.ThrowUnexpectedSubtype(value);
		Gauge gauge = value.Gauge;
		state.WriteMessage(1, SerializerFeatures.CategoryRepeated, gauge, SerializerCache.Get<GaugeSerializer, Gauge>());
		int label = value.Label;
		if (label != 0)
		{
			state.WriteInt32Varint(2, label);
		}
	}
	...
	[SpecialName]
	ISerializer<Gauge> ISerializerProxy<Gauge>.get_Serializer()
	{
		//Error decoding local variables: Signature type sequence must have at least one element.
		return SerializerCache.Get<GaugeSerializer, Gauge>();
	}
}
