...
#pragma warning restore RCS1163, IDE0060 // Unused parameter.
        ...
        internal static object ReadTypedObject(ProtoReader reader, ref State state, object value, int key, Type type)
        {
            ...
            else if (type != null && reader._model.TryDeserializeAuxiliaryType(reader, ref state, DataFormat.Default, TypeModel.ListItemTag, type, ref value, true, false, true, false, null))
            {
                // ok
            }
            else
...
#pragma warning restore RCS1163
        ...
        public T Deserialize<T>(ref State state, T value = default, IProtoDeserializer<T> serializer = null)
        {
            ...
            var result = (serializer ?? TypeModel.GetBasicDeserializer<T>(Model)).Deserialize(this, ref state, value);
