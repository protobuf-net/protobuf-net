...
namespace ProtoBuf
{
    internal partial class ZigZagSerializer : ISerializer<int>, ISerializer<long>
    {
        ...
        public static long ReadInt64(SerializationContext context)
        {
            ...
            val = (-(val & 0x01)) ^ ((val >> 1) & ~Base128Variant.Int64Msb);
            return val;
        }

        internal static int WriteToStream(long value, SerializationContext context)
        {
            return TwosComplementSerializer.WriteToStream(ZigInt64(value), context);
        }
    }
}
