...
- compatibiliy level API: this extends the `DataFormat.WellKnown` concept from v2 allowing granular changes to how common data types are serialized;
  this is discussed [in more depth here](https://docs.protobuf-net.dev/compatibilitylevel) - thanks here go to [ServiceStack](https://servicestack.net/), who
...
- split libraries: as part of the AOT drive, the library has now been split into [`protobuf-net`](https://www.nuget.org/packages/protobuf-net)
  ...
  this in action already at [protobuf-net.dev](https://protobuf-net.dev/), which uses the schema parsing tools *and serializer core* in the client browser
  (not at the web-server) via WASM, via a dependency on `protobuf-net.Core`
...
- minor: `DataFormat.WellKnown` now advises you to use `CompatibilityLevel` instead; if you're happy with your code "as is", you can safely igore this warning (`#pragma` or similar),
  and your code will continue to work in `Level240` (as [discussed here](https://docs.protobuf-net.dev/compatibilitylevel))
