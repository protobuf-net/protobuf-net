...
**THE RULE, after two measured questions from Marc:** a gated arm that wants ILC to delete the rest
of the method must **terminate in IL**. Two forms do, identically; one does not.

| form | unique | total | bytes |
| --- | ---: | ---: | ---: |
| `throw Create…(x)` at the site | **8** | **8** | **3,839,488** |
| `ThrowHelper(x); return default;` | **8** | **8** | **3,839,488** |
| `ThrowHelper(x);` marked `[DoesNotReturn]`, no return | 12 | 13 | 3,887,104 |

**`[DoesNotReturn]` does not substitute for the `return`.** It is a **C# flow-analysis** annotation —
it makes the *compiler* treat following code as unreachable for definite-assignment and nullability
purposes, and it does not change the emitted IL. ILC's reachability is over IL, so the body below a
bare `call` survives and every demand in it survives with it: four warnings and 47,616 bytes worse.
(It is also not used anywhere in Core and has no netstandard2.0 polyfill, so adopting it would have
cost one for no gain.)

**The helper-plus-`return` form is what the codebase uses**, since it keeps the existing
`ThrowHelper` convention and measures the same. Noted at the helper itself so the next person does
not retry the bare call.
...
Marc: *"the thing about reflection is that it has code-generated models, so either we need to
exclude those or fix the codegen."* Neither, as it turns out. Probed with a four-file experiment
(identical code, differing only in header/directive) reading the **emitted metadata**, not just the
warnings:

| file state | NRT warnings | emitted annotations |
| --- | --- | --- |
| no header, no directive | reported | non-nullable (the project setting) |
| `// <auto-generated>` header only | **suppressed** | **oblivious** (no `NullableContextAttribute`) |
| header + `#nullable disable` | suppressed | oblivious |
| header + `#nullable enable` | **reported** | annotated as written |

So a generated file is **outside the nullable context entirely** - it neither warns *nor annotates*,
whatever the project says - and an explicit directive overrides that in either direction. Three
consequences, two of which corrected a wrong conclusion of mine reached an hour earlier:

- **Reflection can be NRT-enabled with zero codegen work.** `Descriptor.cs` and `CustomOptions.cs`
  stay oblivious exactly as today, so nothing false is recorded into its 1065-line baseline. I had
  claimed enabling it would bake non-nullable annotations onto optional message members; that was
  wrong, and only checking the metadata rather than the warnings showed it.
- **Emitting `#nullable disable` from protogen would be a no-op**, since the header already does it.
- **Nothing a CONSUMER configures can annotate protogen output.** Today every protogen user with NRT
  on gets oblivious DTOs, and setting `<Nullable>enable</Nullable>` in their own project changes
  nothing. Only protogen emitting `#nullable enable` will. That is what makes the codegen item worth
  doing on its own merits rather than as NRT-rollout housekeeping.

It is also **self-checking once opted in** - a deliberately wrong annotation in an opted-in generated
file does warn - which cuts both ways: our fixtures would catch mistakes, and a wrong emission puts
warnings in the *consumer* build. The bar is "get it right in the generator".

#### What the codegen change actually has to do

**`ShouldSerializeFoo()` does NOT universally mean "Foo is non-null"**, so a blanket
`MemberNotNullWhen` would be wrong. `CSharpCodeGenerator` has four member shapes and the answer
differs per shape:

| shape | property | what `ShouldSerialize` means | annotation |
| --- | --- | --- | --- |
| **`oneof`** | `T?`; getter is `union.Is(n) ? (T)union.Object : default` | exactly "non-null" | **`[MemberNotNullWhen(true, nameof(X))]`** |
| **`trackPresence`, no default, string/bytes** | `string?`; getter returns the field | `__pbn__X != null` | **`[MemberNotNullWhen(true, nameof(X))]`** |
| **`trackPresence` with a default** (`get => __pbn__Name ?? ""`) | already non-null | "was explicitly SET" | **none** - nothing to narrow, and it would assert something the method does not mean |
| **plain `{ get; set; }`** (message members, e.g. `FileOptions Options`) | needs `?` | no `ShouldSerialize` is emitted | just the `?` |

Note the third row is most of `Descriptor.cs`. It is the same trap the AOT side already records for
`Specified`: the condition *replaces* the trivial-value guard rather than describing null.
`Reset{Name}()` has no counterpart - there is no "member is null on return" post-condition attribute.
...
1. **language version >= C# 8** - and this one is a *veto*, because `#nullable` is itself a C# 8
   ...
   compile error in the consumer build, where today the down-level output is fine precisely because
   the `<auto-generated>` header needs no language support. `CSharpCodeGenerator` declares
   `CSharp6/7/7_1` "only mentioning features we use", so `CSharp8` has to be added, and the check is
   the existing `ctx.Supports(...)`;
